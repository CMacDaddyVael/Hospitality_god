    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          {plan ? (
            ...
            <p className="text-gray-500 mt-1">Your AI marketing team&apos;s weekly output</p>
          )}
        </div>
        <button
          onClick={generatePlan}
          disabled={loading}
          className="flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded-lg text-sm font-medium hover:bg-green-400 transition disabled:opacity-50"
        >
          {loading ? (
            <>
      ...
      {!plan && !loading && (
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <button
            onClick={generatePlan}
            className="px-6 py-3 bg-green-500 text-white rounded-xl font-medium hover:bg-green-400 transition"
          >
            ✨ Generate This Week&apos;s Content
          </button>
        </div>
      ...
      {deliverables.length > 0 && (
        <div className="flex gap-2 mb-6">
          {[
            ...
            { key: "listing-update", label: "✍️ Listing" },
            ...
            >
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {item.suggestedDay && (
                    <span className="text-xs text-gray-400">📅 {item.suggestedDay}</span>
                  )}
                  {item.priority === "high" && (
              ...
              {item.reason && (
                <p className="text-sm text-gray-400 mb-3 italic">💡 {item.reason}</p>
              )}
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {(item.type === "social" || item.type === "seasonal") && !item.generatedImageUrl && item.scene && (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-4 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-500 transition disabled:opacity-50"
                    >
